        ...
        vueSelectUpdated(value) {
            if (value) {
                if (this.config.storage_options === 'both' || this.config.storage_options === 'url') {
                    this.update({iconPrefix: value.value.split(":")[0],iconName: value.value.split(":")[1], iconUrl: value.src, iconSvg: null, label: value.value});
                }
                ...
                if (this.config.storage_options === 'both' || this.config.storage_options === 'svg') {
                    fetch(`${value.src}?width=unset&height=unset`).then(res => res.text()).then(svg => {
                        this.update({iconName: value.value, iconUrl: (this.config.storage_options === 'both' ? value.src : null), iconSvg: svg, label: value.value});
                    });
                }
            } else {
                this.update(null);
